        ...
        let run_replay = move || {
            ...
            loop {
                ...
                for slot in newly_computed_slot_stats {
                    ...
                    let (duplicate_confirmed_forks, mostly_confirmed_forks) = Self::tower_duplicate_confirmed_forks(
                        &tower,
...
pub(crate) mod tests {
    ...
    fn test_replay_stage_last_vote_outside_slot_hashes() {
        ...
        let mut junk = 0;let SelectVoteAndResetForkResult { vote_bank, .. } = select_vote_and_reset_forks(
            other_fork_bank,
            Some(&new_bank),
            &bank_forks.read().unwrap().ancestors(),
            &bank_forks.read().unwrap().descendants(),
